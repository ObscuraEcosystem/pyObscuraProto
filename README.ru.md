...
Наблюдаемый статус каждого сценария зафиксирован в таблице `EXPECTED` внутри `run_audit.py` (базлайн v1.1.1, 14 записей). Раннер завершается с кодом `1` при любом расхождении (включая любой `UNKNOWN`) и с `0` при полном совпадении — это и отображает job `audit` в CI. Текущие записи `FAIL` в `EXPECTED` — **временные плейсхолдеры** с пометкой `TODO`: эти сценарии ещё не доработаны, поэтому красный job `audit` не обязательно означает регрессию. Запуск аудита локально и обновление `EXPECTED` описаны в [docs/development_guide.md](docs/development_guide.md).
