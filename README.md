...
The observed status of every scenario is frozen in the `EXPECTED` table inside `run_audit.py` (baseline v1.1.1, 14 entries). The runner exits `1` on any divergence (including any `UNKNOWN`) and `0` when everything matches — this is what the CI `audit` job reports. All 14 scenarios now assert real behavior and `EXPECTED` is green: X1–X3 PASS, X4-P1..P6 PASS, X4-P7 HANG (self-declared — the client-side timeout is 8 s), X5-S1..S3 PASS, X6 PASS. X6 asserts five timeout semantics (distances ±1 s, `ObscuraProto.TimeoutError` type); X4-P6 asserts `LogicError` "Session not ready". Known gaps: X5-S2 is redesigned — GC during an **active callback** is not exercised (the server does not dispatch handlers under `asyncio.run`), instead the scenario tests GC during an in-flight request; X6 is the only timing-sensitive scenario (risk ~5% on a loaded CI at point X6-1b). See [docs/development_guide.md](docs/development_guide.md) for running the audit locally and updating `EXPECTED`.
